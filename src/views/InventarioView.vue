  ...
  <div class="page-wrapper">
    ...
    <main class="content">
      ...
      <div class="filters-bar">
        <input v-model="busquedaCaja" type="text" placeholder="Buscar por comentarios…"
          class="search-input" @input="fetchCajas" />
        <select v-model="filtroSexoCaja" class="filter-select" @change="fetchCajas">
      ...
      <div class="table-card">
        ...
        <table v-else class="data-table">
          <thead>
            <tr>
              <th>Caja #</th><th>Ratas totales</th><th>Sexo</th>
              <th>F. Nacimiento</th><th>Talla</th><th>Responsable</th>
              <th>Comentarios</th>
      ...
      <div class="filters-bar">
        <input v-model="busquedaRata" type="text" placeholder="Buscar por número de cola…"
          class="search-input" @input="fetchRatas" />
        <select v-model="filtroSexoRata" class="filter-select" @change="fetchRatas">
          <option value="">Todos los sexos</option>
          <option value="Macho">Macho</option>
          <option value="Hembra">Hembra</option>
        </select>
        <select v-model="filtroCaja" class="filter-select" @change="fetchRatas">
      ...
      <div class="table-card">
        ...
        <table v-else class="data-table">
          <thead>
            <tr>
              <th>ID</th><th>Sexo</th><th>N° Cola</th><th>Caja</th>
              <th>Condición</th><th>Peso semanal (g)</th><th>F. Cirugía</th>
              <th v-if="isEncargado">Acciones</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="r in ratas" :key="`${r.sexo}-${r.idrata}`">
              ...
              <td><span v-if="r.condicion_nombre" class="badge badge-purple">{{ r.condicion_nombre }}</span><span v-else>—</span></td>
              <td>{{ r.pesosemanal ?? '—' }}</td>
          ...
          <form @submit.prevent="guardarRata">
            <div class="fields-grid">
              ...
              <div class="field">
                ...
                <div class="id-row">
                  <input
                    v-model.number="formRata.idrata"
                    type="number" min="1"
                    :placeholder="formRata.sexo ? `Auto: ${siguienteId}` : 'Selecciona sexo primero'"
                    :disabled="!formRata.sexo || !isEncargado"
                    :class="{ 'input-error': errorIdRata }"
                  />
                  <button
                    v-if="formRata.sexo && isEncargado"
                    type="button"
                    class="btn-auto"
                    @click="usarIdAuto"
                    title="Usar ID automático"
                  >Auto</button>
                </div>
...
export default {
  ...
  computed: {
    ...
    cajasFiltradas() {
      if (!this.formRata.sexo) return this.cajas
      return this.cajas.filter(c => c.sexo === this.formRata.sexo)
    },
  },
  async created() {
    await Promise.all([this.fetchCajas(), this.fetchRatas(), this.fetchCondiciones(), this.fetchUsuarios()])
  },
  methods: {
    // ── Cajas ─────────────────────────────────────────────────
    async fetchCajas() {
      this.loadingCajas = true; this.errorCajas = null
      try {
        const params = {}
    ...
    limpiarCajas() { this.busquedaCaja = ''; this.filtroSexoCaja = ''; this.fetchCajas() },
    ...
    // ── Ratas ──────────────────────────────────────────────────
    async fetchRatas() {
      this.loadingRatas = true; this.errorRatas = null
      try {
        const params = {}
    ...
    limpiarRatas() { this.busquedaRata = ''; this.filtroSexoRata = ''; this.filtroCaja = ''; this.fetchRatas() },
    ...
    async cargarSiguienteId() {
      ...
      try {
        ...
        if (!this.editandoRata && !this.formRata.idrata) {
...
.section-divider { height: 2px; background: #e8e8ee; margin: 2.5rem 0; border-radius: 2px; }
.page-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.1rem; }
.page-header h1 { font-size: 1.4rem; font-weight: 600; color: #1a1a2e; margin: 0; }
.filters-bar { display: flex; gap: 10px; margin-bottom: 1.1rem; flex-wrap: wrap; }
.search-input { flex: 1; min-width: 160px; padding: .55rem .9rem; border: 1.5px solid #ddd; border-radius: 8px; font-size: .9rem; outline: none; }
.search-input:focus { border-color: #80201d; }
.filter-select { padding: .55rem .8rem; border: 1.5px solid #ddd; border-radius: 8px; font-size: .9rem; background: #fff; outline: none; }
.table-card { background: #fff; border-radius: 12px; border: 1px solid #e0e0e0; overflow: hidden; margin-bottom: 1rem; overflow-x: auto; }
.data-table { width: 100%; border-collapse: collapse; font-size: .87rem; }
.data-table th { background: #f8f8fb; padding: .65rem 1rem; text-align: left; font-size: .77rem; font-weight: 600; color: #666; border-bottom: 1px solid #e0e0e0; white-space: nowrap; }
.data-table td { padding: .6rem 1rem; border-bottom: 1px solid #f0f0f0; color: #333; }
...
.badge { display: inline-block; font-size: .75rem; padding: 2px 10px; border-radius: 99px; font-weight: 500; }
.badge-blue   { background: #e3f2fd; color: #1565c0; }
.badge-pink   { background: #fce4ec; color: #c62828; }
.badge-purple { background: #f3e5f5; color: #6a0dad; }
.state-msg { padding: 2rem; text-align: center; color: #888; }
.state-msg.error { color: #c62828; }
.btn-primary { background: #80201d; color: #fff; border: none; padding: .55rem 1.2rem; border-radius: 8px; font-size: .9rem; font-weight: 500; cursor: pointer; }
.btn-primary:hover:not(:disabled) { background: #9e2a26; }
.btn-primary:disabled { opacity: .6; cursor: not-allowed; }
.btn-secondary { background: transparent; border: 1.5px solid #ccc; color: #555; padding: .55rem 1.2rem; border-radius: 8px; font-size: .9rem; cursor: pointer; }
...
.modal h2 { font-size: 1.15rem; font-weight: 600; margin: 0 0 1.5rem 0; color: #1a1a2e; }
...
.field input, .field select, .field textarea { padding: .55rem .8rem; border: 1.5px solid #ddd; border-radius: 8px; font-size: .9rem; outline: none; font-family: inherit; resize: vertical; }
.field input:focus, .field select:focus { border-color: #80201d; }
.input-error { border-color: #c62828 !important; background: #fff8f8; }
.field-error { color: #c62828; font-size: .78rem; }
.field-hint { color: #aaa; font-size: .76rem; }
.label-hint { font-size: .75rem; color: #80201d; font-weight: 400; margin-left: 6px; }
.req { color: #c62828; }
.id-row { display: flex; gap: 6px; }
.id-row input { flex: 1; }
.btn-auto { padding: .4rem .8rem; background: #f0e8e8; color: #80201d; border: 1.5px solid #d4a0a0; border-radius: 7px; font-size: .8rem; font-weight: 500; cursor: pointer; white-space: nowrap; }
.btn-auto:hover { background: #e8d0d0; }
.form-error { color: #c62828; font-size: .82rem; background: #ffebee; padding: .5rem; border-radius: 6px; margin-top: .75rem; }

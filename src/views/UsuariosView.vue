            ...
            <div class="fields-grid">
              ...
              <div class="field">
                ...
                <input v-model="form.password_confirm" type="password"
                  :required="!editando || !!form.password"
              ...
              <div class="field">
                ...
                <input v-model="form.apellidopaterno" required
                  :class="{ 'input-error': fieldErrors.apellidopaterno }" />
                <span v-if="fieldErrors.apellidopaterno" class="field-error">
              ...
              <div class="field">
                ...
                <input v-model="form.apellidomaterno" required
                  :class="{ 'input-error': fieldErrors.apellidomaterno }" />
              </div>
              ...
              <div class="field">
                ...
                <select v-model="form.idrol" required
                  :class="{ 'input-error': fieldErrors.idrol }">
                  <option value="">Seleccionar…</option>
                  <option v-for="r in roles" :key="r.idrol" :value="r.idrol">
                    {{ r.nombrerol }}
                  </option>
                </select>
                <span v-if="fieldErrors.idrol" class="field-error">
                  {{ fieldErrors.idrol }}
                </span>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async guardarUsuario() {
      ...
      try {
        // Construir payload: solo incluir password si tiene valor
        const payload = {
        ...
        if (data && typeof data === 'object') {
          // El backend devuelve errores por campo → mostrarlos inline
          this.fieldErrors = data
          // Si hay un error genérico (non_field_errors)
          if (data.non_field_errors) {
            this.formError = data.non_field_errors.join(' ')
          }
        } else {
          this.formError = 'Error al guardar. Verifica los datos e inténtalo de nuevo.'
        }
      } finally {
        this.saving = false
      }
...
.data-table th { background: #f8f8fb; padding: 0.75rem 1rem; text-align: left; font-size: 0.8rem; font-weight: 600; color: #666; border-bottom: 1px solid #e0e0e0; }
.data-table td { padding: 0.7rem 1rem; border-bottom: 1px solid #f0f0f0; color: #333; }
.data-table tr:last-child td { border-bottom: none; }
.empty-row { text-align: center; color: #aaa; padding: 2rem !important; }
.actions { display: flex; gap: 6px; }
.role-chip { background: #f3e5f5; color: #6a0dad; font-size: 0.78rem; padding: 2px 8px; border-radius: 99px; }
...
.modal { background: #fff; border-radius: 14px; padding: 2rem; width: 100%; max-width: 520px; max-height: 90vh; overflow-y: auto; }
.modal h2 { font-size: 1.2rem; font-weight: 600; margin: 0 0 1.5rem 0; color: #1a1a2e; }
...
.field input, .field select { padding: 0.55rem 0.8rem; border: 1.5px solid #ddd; border-radius: 8px; font-size: 0.9rem; outline: none; }
.field input:focus, .field select:focus { border-color: #80201d; }
.input-error { border-color: #c62828 !important; background: #fff8f8; }
.field-error { color: #c62828; font-size: 0.78rem; margin-top: 2px; }
.label-hint { font-size: 0.75rem; color: #999; font-weight: 400; margin-left: 4px; }
.form-error { color: #c62828; font-size: 0.82rem; background: #ffebee; padding: 0.5rem; border-radius: 6px; margin-top: 0.5rem; }
